import React, { useContext, useState } from "react"
import axios from 'axios'

const BASE_URL = 'http://localhost:8020/api/v1/';

const GlobalContext = React.createContext()

export const GlobalProvider = ({children}) =>{
    
    const [incomes, setIncomes] = useState([])
    const [expenses, setExpenses] = useState([])
    const [error, setError] = useState(null)
    const [editIncomeData, setEditIncomeData] = useState(null); 
    const [editExpenseData, setEditExpenseData] = useState(null); 

    //calculate incomes
    
    const addIncome = async (income) =>{
        const response = await axios.post(`${BASE_URL}add-income`, income)
        .catch((err) =>{
            setError(err.response.data.message)
        })
        getIncomes()
    }

    const updateIncome = async (id, updatedIncome) => {
        try {
            await axios.put(`${BASE_URL}put-income/${id}`, updatedIncome); // Adjust endpoint to match your API
            getIncomes(); // Fetch updated incomes after edit
            setEditIncomeData(null); // Clear edit data after successful update
        } catch (err) {
            setError(err.response?.data?.message || "Error updating income");
        }
    };

    const getIncomes = async () => {
        const response = await axios.get(`${BASE_URL}get-incomes`)
        setIncomes(response.data)
        console.log(response.data)
    }

    const deleteIncome = async (id) => {
        const res  = await axios.delete(`${BASE_URL}delete-income/${id}`)
        getIncomes()
    }

    const totalIncome = () => {
        let totalIncome = 0;
        incomes.forEach((income) =>{
            totalIncome = totalIncome + income.amount
        })

        return totalIncome;
    }
    
    //calculate expences

    const addExpense = async (income) => {
        const response = await axios.post(`${BASE_URL}add-expense`, income)
            .catch((err) =>{
                setError(err.response.data.message)
            })
        getExpenses()
    }

    const updateExpense = async (id, updatedExpense) => {
        try {
            await axios.put(`${BASE_URL}put-expense/${id}`, updatedExpense); 
            getExpenses(); 
            setEditExpenseData(null); 
        } catch (err) {
            setError(err.response?.data?.message || "Error updating Expense");
        }
    };

    const getExpenses = async () => {
        const response = await axios.get(`${BASE_URL}get-expenses`)
        setExpenses(response.data)
        console.log(response.data)
    }

    const deleteExpense = async (id) => {
        const res  = await axios.delete(`${BASE_URL}delete-expense/${id}`)
        getExpenses()
    }

    const totalExpenses = () => {
        let totalIncome = 0;
        expenses.forEach((income) =>{
            totalIncome = totalIncome + income.amount
        })

        return totalIncome;
    }

    const totalBalance = () => {
        return totalIncome() - totalExpenses()
    }

    const transactionHistory = () => {
        const history = [...incomes, ...expenses]
        history.sort((a, b) => {
            return new Date(b.createdAt) - new Date(a.createdAt)
        })

        return history.slice(0, 3)
    }

    const setIncomeToEdit = (income) => {
        setEditIncomeData(income); 
    };

    const setExpenseToEdit = (expense) => {
        setEditExpenseData(expense); 
    };


    
    return(
        <GlobalContext.Provider value = {{
            addIncome,
            getIncomes,
            incomes,
            deleteIncome,
            expenses,
            updateIncome, 
            setIncomeToEdit,
            setExpenseToEdit, 
            editIncomeData,
            totalIncome,
            addExpense,
            updateExpense,
            editExpenseData,
            getExpenses,
            deleteExpense,
            totalExpenses,
            totalBalance,
            transactionHistory,
            error,
            setError

        }}>

            {children}
        </GlobalContext.Provider>
    )
}

export const useGlobalContext = () =>{
    return useContext(GlobalContext)
}
